...
- 匯出 PDF：把逐字稿畫成 HTML 後用 `html2canvas` 截圖、`jsPDF` 組成 PDF（借用系統中文字型，避免額外打包字型檔）
  - 分頁時會先量出每一行的位置，只在行與行的空隙下刀，不會把文字攔腰切斷
  - 每頁以 JPEG 嵌入；若用 PNG，jsPDF 會以未壓縮點陣存入，一頁就要 10MB 左右
- **併發處理**：多個片段同時送去辨識（預設 3 個），不是一段做完才做下一段。一小時的會議約可縮短到原本的 1/3 時間。結果會依時間順序重新組合，不會亂序
  - 併發數定義在 `src/lib/chunkRunner.ts` 的 `DEFAULT_CONCURRENCY`，調高更快但比較容易撞到 API 頻率限制（429）
...
- **斷點續傳**：轉錄進度（已完成到第幾段、目前的逐字稿）會存在瀏覽器 `localStorage`。如果中途網路斷線、分頁關掉、或重試 3 次後還是失敗，只要**重新拖上同一個檔案**（檔名、大小、修改時間都相同）就會從中斷的地方接著轉錄，不用整份重來
  ...
  - 因為是併發處理，存檔只記錄「從頭連續完成」的進度。例如第 3 段好了但第 2 段還沒好時，續傳點仍停在第 2 段，確保中斷後不會漏掉中間的段落
